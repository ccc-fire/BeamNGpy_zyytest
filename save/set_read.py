...
def collection_time(collection_time, data_collection):
    start_time = time.time()
    while True:
        cur_time = time.time()
        dura_time = cur_time - start_time
        print(f"已经采集 {dura_time} seconds")

        if dura_time >= 30:  # 设置采集时间为30秒钟
            break
        data_collection()
    print("数据采集完成，程序退出")
...
def main():
    ...
    try:
        ...
        camera_front = Camera('camera_front', bng, vehicle_1,
                          pos=(0.000244140625, -0.049774169921875, 1.465557861328125), dir=(0, -1, 0), field_of_view_y=120, resolution=resolution,
        ...
        camera_front_left = Camera('camera_front_left', bng, vehicle_1,
                          pos=(0.3402506510416667, -0.049774169921875, 1.465557861328125), dir=(0, -1, 0), field_of_view_y=120, resolution=resolution,
                          is_render_colours=True, is_render_depth=True, is_render_annotations=True,
                          is_render_instance=True)
        camera_front_right = Camera('camera_front_right', bng, vehicle_1,
                          pos=(-0.3397623697916667, -0.049774169921875, 1.465557861328125), dir=(0, -1, 0), field_of_view_y=120, resolution=resolution,
                          is_render_colours=True, is_render_depth=True, is_render_annotations=True,
                          is_render_instance=True)
        camera_back_left = Camera('camera_back_left', bng, vehicle_1,
                          pos=(0.408251953125, 0.884173583984375, 1.465557861328125), dir=(0, 1, 0), field_of_view_y=120, resolution=resolution,
                          is_render_colours=True, is_render_depth=True, is_render_annotations=True,
                          is_render_instance=True)
        camera_back_right = Camera('camera_back_right', bng, vehicle_1,
                          pos=(-0.407763671875, 0.884173583984375, 1.465557861328125), dir=(0, 1, 0), field_of_view_y=120, resolution=resolution,
                          is_render_colours=True, is_render_depth=True, is_render_annotations=True,
                          is_render_instance=True)
        camera_back = Camera('camera_back', bng, vehicle_1,
                          pos=(0.000244140625, 1.506805419921875, 1.465557861328125), dir=(0, 1, 0), field_of_view_y=120, resolution=resolution,
                          is_render_colours=True, is_render_depth=True, is_render_annotations=True,
                          is_render_instance=True)
        ...
        lidar_Top = Lidar('lidar_Top', bng, vehicle_1, pos = (0.000244140625, 1.1176605224609375, 1.665557861328125), requested_update_time=0.01, is_using_shared_memory=False)


        # set IMU

        IMU = AdvancedIMU('IMU', bng, vehicle_1, pos=(0.000244140625, 1.2844369070870536, 1.465557861328125), gfx_update_time=0.01)
        ...
        radar_front_right = Radar('radar_front_right', bng, vehicle_1, requested_update_time=0.01,
                       pos=(-1.019775390625, -1.2172088623046875, 0.8180770874023438), dir=(0, -1, 0), up=(0, 0, 1),
                       resolution=(200, 200), field_of_view_y=70, near_far_planes=(0.1, 100.0),
                       range_roundess=-2.0, range_cutoff_sensitivity=0.0, range_shape=0.23, range_focus=0.12,
                       range_min_cutoff=0.5, range_direct_max_cutoff=100.0, is_snapping_desired=True)

        radar_front_left = Radar('radar_front_left', bng, vehicle_1, requested_update_time=0.01,
                       pos=(1.020263671875, -1.2172088623046875, 0.8180770874023438), dir=(0, -1, 0), up=(0, 0, 1),
                       resolution=(200, 200), field_of_view_y=70, near_far_planes=(0.1, 100.0),
                       range_roundess=-2.0, range_cutoff_sensitivity=0.0, range_shape=0.23, range_focus=0.12,
                       range_min_cutoff=0.5, range_direct_max_cutoff=100.0, is_snapping_desired=True)
        ...
        while True:
            ...
            for frame_id in range(11):
                ...
                print(f"已经采集 {elapsed_time} seconds")

                if elapsed_time >= 30:  # 设置采集时间为10秒钟
                ...
                end1_time = time.time()
                execution_time = end1_time - start1_time  # 计算代码执行时间

                print(f"frame_id: {frame_id} start: {start1_time}, end:{end1_time}, totol {execution_time} seconds")
