...
def collect_camera(camera_name: Camera, frame_id):
...
def collect_radar(radar_name: Radar, frame_id):
...
def main():
    ...
    try:
        ...
        camera_front = Camera('camera_front', bng, vehicle_1,
                              ...
                              field_of_view_y=120, resolution=resolution, is_using_shared_memory=True,
        ...
        bird_view_camera = Camera('bird_view_camera', bng, pos=(7073, 2765, 200),
        ...
        #                   requested_update_time=0.01, is_using_shared_memory=False, is_visualised=False)
        #
        # # set IMU
        ...
        for frame_id in range(2):
            ...
            # # save radar data 雷达
            # collect_radar(radar_front, frame_id)
            # collect_radar(radar_back_left, frame_id)
            # collect_radar(radar_front_left, frame_id)
            # collect_radar(radar_back_right, frame_id)
            # collect_radar(radar_front_right, frame_id)
            #
            # t1 = time.time()
            ...
            # t4 = time.time()
            # print(f"7个camera(poll-save){resolution}： {t4 - start_time} seconds")
            end_time = time.time()
            print(f"7个camera(poll-save){resolution} 第{frame_id}轮的时间： {end_time - start_time} seconds")
            # 生成统计报告
